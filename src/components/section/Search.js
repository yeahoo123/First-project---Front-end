...
export class Products extends Component {
    ...
    render() {
        const {products, searchName} = this.context 
        const data = searchName.toLowerCase()
        const search = products.filter(product => product.product_name.toLowerCase().match(data))
        console.log(search)
        if(search.length === 0){
            return <h2 style={{textAlign: "center"}}>Không có sản phẩm cần tìm</h2>
        }else {
            return (
                <div id="product">
